...
{   
    char header[54];
    //go with fread because .bmp is image data
...
{
    //run loop for size number of times
    for(int i=0;i<size;i++)
    {
        //read 8 btyes of RGB from source image
        fread(encInfo->image_data,8,sizeof(char),fptr_src);
        //call encode_byte_to_LSB function
        encode_byte_to_lsb(data[i],encInfo->image_data);
        //after encoding the data will be in the same array, write that into destination image
...
{
    //every character encoding should call encode_data_to_image
...
Status do_encoding(EncodeInfo *encInfo)
{
    if(open_files(encInfo) == e_success)
    {
        ...
        {
            printf("Image has enough RGB data to encode.. u can proceed\n ");
             if(copy_bmp_header(encInfo->fptr_src_image,encInfo->fptr_stego_image) == e_success)
             {
                ...
                if(encode_magic_string(MAGIC_STRING,encInfo) == e_success)
                {
                    ...
                    {
                        printf("Sucessfully encoded the secret file extension size\n");
                        ...
                        {
                            printf("Faile to encode the secret file extension \n ");
                            return e_failure;
                        }
                    }
                    else
        ...
        {
            printf("Enough RGB is not available to do the encoding\n");
            return e_failure;
        }
    }
    else
